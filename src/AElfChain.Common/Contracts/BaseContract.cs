...
namespace AElfChain.Common.Contracts
{
    public class BaseContract<T>
    {
        ...
        protected BaseContract(INodeManager nodeManager, string fileName, string callAddress, string salt = "", bool isApprove = true)
        {
            ...
            {
                DeployContractWithoutApproval(callAddress, salt);
            }
        }
        ...
        private void DeployContractWithoutApproval(string account, string salt)
        {
            Logger.Info($"Deploy contract without authority mode. {FileName}");
            var genesisContract = new GenesisContract(NodeManager, account, NodeManager.GetGenesisContractAddress());
            var authority = new AuthorityManager(NodeManager, account);
            var parliament = genesisContract.GetParliamentContract(account);
            var result = genesisContract.DeployUserSmartContract(FileName, HashHelper.ComputeFrom(salt), account);
            ...
            parliament.CheckDeployOrUpdateProposal(proposalId, authority.GetCurrentMiners());
            var currentHeight = AsyncHelper.RunSync(NodeManager.ApiClient.GetBlockHeightAsync);
            var release = genesisContract.FindReleaseApprovedUserSmartContractMethod(result.BlockNumber, currentHeight);
            ...
            var contractInfo = genesisContract.CallViewMethod<ContractInfo>(GenesisMethod.GetContractInfo,
                contractDeployedNonIndexed.Address);
            Logger.Info($"Deploy Contract Info: {contractInfo}");
            ContractAddress = contractDeployedNonIndexed.Address.ToBase58();
        }
